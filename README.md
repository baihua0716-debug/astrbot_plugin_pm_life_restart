...
随后用数字选择四个天赋、分配属性并选择继承，不需要反复输入 `/月计人生`。私聊可直接发送数字；群聊需引用回复 Bot 的提示，或 @Bot 后输入数字，避免群成员的普通数字消息被误当作游戏操作。快捷回复在提示后的 10 分钟内有效；超时发送 `/月计人生` 可查看当前步骤并继续。插件会一次性推演完整人生，通过 QQ 合并转发发送轨迹。群聊存档以“当前群 + QQ 号”隔离；私聊使用单独的个人存档，不与任何群共享。
